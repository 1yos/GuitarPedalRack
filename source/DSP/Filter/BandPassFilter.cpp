...
BandPassFilter::BandPassFilter() : AudioModule("BandPassFilter") {}
...
void BandPassFilter::setFrequency(float frequency) { frequencyParam.store(frequency); }
void BandPassFilter::setBandwidth(float bandwidth) { bandwidthParam.store(bandwidth); }
void BandPassFilter::setGain(float gain) { gainParam.store(gain); }

void BandPassFilter::processInternal(AudioBuffer<float>& buffer)
{
    const float freq = frequencyParam.load() * 9000.0f + 100.0f;
    const float BW = bandwidthParam.load() * 3.0f + 0.1f;
    const float gain = gainParam.load() * 2.0f;
    ...
    const float b0 = alpha * gain;
    const float b1 = 0.0f;
    const float b2 = -alpha * gain;
