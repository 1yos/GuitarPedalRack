...
LowPassEQ::LowPassEQ() : AudioModule("LowPassEQ")
{
}
...
void LowPassEQ::prepare(double sampleRate, int samplesPerBlock)
{
...
void LowPassEQ::setFrequency(float freq)
{
    frequencyParam.store(jlimit(0.0f, 1.0f, freq), std::memory_order_relaxed);
    updateCoefficients();
}

void LowPassEQ::setResonance(float res)
{
    resonanceParam.store(jlimit(0.0f, 1.0f, res), std::memory_order_relaxed);
    updateCoefficients();
}

void LowPassEQ::setSlope(float slope)
{
    slopeParam.store(jlimit(0.0f, 1.0f, slope), std::memory_order_relaxed);
}

void LowPassEQ::updateCoefficients()
{
    const float fc = 200.0f + frequencyParam.load(std::memory_order_relaxed) * 19800.0f; // 200-20000Hz
...
void LowPassEQ::processInternal(AudioBuffer<float>& buffer)
{
    if (!bypassed)
    {
